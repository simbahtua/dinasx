<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01//EN"
    "http://www.w3.org/TR/html4/strict.dtd">

<!--[if IE 8]> <html lang="en" class="ie8 no-js"> <![endif]-->
<!--[if IE 9]> <html lang="en" class="ie9 no-js"> <![endif]-->
<!--[if !IE]><!--> <html lang="en" class="no-js"> <!--<![endif]-->

<!-- BEGIN HEAD -->
<head>
    <meta charset="utf-8" />
    <title>Pemerintah Kabupaten Bantaeng</title>

    <!-- Always force latest IE rendering engine (even in intranet) & Chrome Frame -->
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <!-- Mobile viewport optimized: h5bp.com/viewport -->
    <meta content="width=device-width, initial-scale=1.0" name="viewport" />
    <meta name="MobileOptimized" content="320" />

    <meta name="description" content="" />
    <meta name="keywords" content="" />
    <meta name="author" content="Air Amanah IT"/>

    <!-- BEGIN GLOBAL MANDATORY STYLES -->          
    <link href="landingpage/css/bootstrap.css" rel="stylesheet" type="text/css"/>
    <!-- END GLOBAL MANDATORY STYLES -->

    <!-- BEGIN THEME STYLES --> 
    <link href="landingpage/css/imports.css" rel="stylesheet" type="text/css"/>
    <link href="landingpage/css/custom.css" rel="stylesheet" type="text/css"/>
    <link href="landingpage/css/responsive.css" rel="stylesheet" type="text/css"/>
    <!-- END THEME STYLES -->

    <!-- JQUERY LIBRARY -->
    <script type='text/javascript' src="landingpage/js/jquery.min.js"></script>
    <!-- JQUERY LIBRARY -->

    <link rel="shortcut icon" href="landingpage/images/favicon.ico" />
</head>
<!-- END HEAD -->

<!-- BEGIN BODY -->
<body>

    <!-- Header -->
    <header>
        <div class="main-nav">
            <div class="container">

                <!-- blok companies -->
                <div id="blok-company">
                    <nav class="navbar navbar-default">
                        <!-- Brand and toggle get grouped for better mobile display -->
                        <div class="navbar-header">
                            <div class="navbar-brand">
                                <a href="#" class="logo" alt="logo">
                                    <img src="landingpage/images/logo.png" alt="Pemerintah Kabupaten Bantaeng">
                                </a>
                                <h1 class="navbar-brand-text">Pemerintah <strong>Kabupaten Bantaeng</strong></h1>
                            </div>
                        </div>
                    </nav>
                </div>
                <!-- blok companies -->
            </div>
        </div>       
    </header>
    <!-- /Header -->
